...
locals {
  ...
  cloud_build_content_hash = sha512(join(",", [
    for f in local.cloud_build_fileset : filesha512("${path.module}/${f}")
  ]))
}
...
resource "local_file" "cloudbuild_config" {
  ...
  content = templatefile("${path.module}/build/cloudbuild.yaml.template", {
    ...
    image_tag             = "${var.region}-docker.pkg.dev/${module.project_services.project_id}/${var.artifact_repo}/${var.adpapi_service_name}"
  })
}

# Build and upload the app container
module "gcloud_build_apd_api_app" {
