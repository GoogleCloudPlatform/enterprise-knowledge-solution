...
def read_classifier_job_output(
    ...
    logging.info(
        f"Found {len(output_blobs)} under bucket {process_bucket} "
        f"with {prefix=}")
    ...
    for blob in output_blobs:
        ...
        original_with_suffix_basename = \
        os.path.basename(blob.name).split(".json")[
            0]
    ...
    for original_blob_path, entities in (
            ...
            map(
                ...
                sorted(
                    filter(
                        ...
                        filter(
                            lambda e: e.type.strip().lower() in known_labels,
                            entities
                            )
        ...
        if not sorted_entities_type_but_only_above_threshold:
            ...
            # output.
            logging.info(
                f"{original_blob_path} was not detected to be "
        ...
        logging.info(
            f"{original_blob_path} was detected to be "
            f"a '{chosen_label}'.")
