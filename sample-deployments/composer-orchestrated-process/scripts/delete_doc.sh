...
usage() {
  ...
  echo "  -r RUN ID     Run ID - the suffix of the data table in bigquery, can be found in the GCS path or from the composer run."
...
gcloud run jobs execute doc-deletion-job --update-env-vars="RUN_ID=$RUN_ID,DOC_ID=$DOC_ID,MODE=$MODE" --wait
result=$?
